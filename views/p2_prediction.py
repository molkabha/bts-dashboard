...
def _render_models_comparison(
    models_df: pd.DataFrame, template: str, prod_name: str
) -> None:
    ...
    with section("Comparaison des modèles"):
        ...
        with tab_r2:
            ...
            colors = [
                "#059669" if str(m) == prod_name else "#1e3a8a"
                for m in chart_df["Modèle"]
            ]
...
def page_prediction():
    ...
    if is_admin():
        ...
        with c2:
            ...
            kpi_card(
                "R²", f"{float(r2):.3f}" if r2 is not None else "—", "Jeu test", "blue"
            )
        ...
        with c3:
            ...
            kpi_card(
                "RMSE",
                f"{float(rmse):.3f} kWh" if rmse is not None else "—",
                "",
                "gray",
            )
    ...
    stations = (
        clean_station_id_list(df["station_id"].unique())
    ...
    if (
        ...
        and valid_station_id(st.session_state.get("pred_station")) is None
    ):
    ...
    if is_admin():
        ...
        if shap_data:
            ...
            with section("Variables cles (SHAP)"):
                ...
                fig = go.Figure(
                    go.Bar(x=values, y=labels, orientation="h", marker_color="#1e3a8a")
                )
