...
def _station_inventory() -> pd.DataFrame:
    ...
    if "Conso moy. (kWh)" in meta.columns:
        ...
        meta["Conso moy. (kWh)"] = pd.to_numeric(
            meta["Conso moy. (kWh)"], errors="coerce"
        ).round(2)
...
def _render_stations_tab():
    ...
    if inventory.empty:
        ...
        st.warning(
            "Aucune station dans le jeu de données actif. Vérifiez les artefacts NB publiés sur Hugging Face."
        )
    ...
    active_count = (
        int(inventory["Actif"].sum()) if "Actif" in inventory.columns else total
    )
    ...
    with k2:
        ...
        kpi_card(
            "Actives", str(active_count), "Visibles dans le tableau de bord", "green"
        )
    ...
    with k3:
        ...
        kpi_card(
            "Désactivées",
            str(inactive_count),
            "Exclues des filtres et cartes",
            "orange",
        )

    st.caption(
        "Les stations désactivées sont masquées pour tous les utilisateurs. Les accès par ingénieur se gèrent dans l'onglet Utilisateurs."
    )
    ...
    with section("Parc stations"):
        ...
        search = st.text_input(
            "Rechercher",
            placeholder="ID station, gouvernorat, technologie…",
            key="cfg_station_search",
        )
        ...
        with bc3:
            ...
            show_inactive_only = st.checkbox(
                "Voir seulement désactivées", value=False, key="cfg_inactive_only"
            )
        ...
        column_config = {
            "Actif": st.column_config.CheckboxColumn(
                "Activée", help="Décochez pour masquer la station"
            ),
        ...
        if st.button("Enregistrer", type="primary", use_container_width=False):
            ...
            if edited is None or edited.empty:
                ...
                new_inactive = (
                    edited.loc[~edited["Actif"].astype(bool), "Station"]
                    .astype(str)
                    .tolist()
                )
            ...
            st.success(
                "Parc stations enregistré. Les vues du tableau de bord sont à jour."
            )
