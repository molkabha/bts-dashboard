...
def _chart_top_stations(df: pd.DataFrame, template: str, *, admin: bool, limit: int = 10) -> None:
    ...
    agg = work.assign(_e=expert, _r=rl).groupby("station_id", as_index=False).agg(
        expert=("_e", "sum"), rl=("_r", "sum"),
    )
...
def _chart_hourly(df: pd.DataFrame, template: str) -> None:
    ...
    if hourly.empty:
        ...
        return
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=hourly["heure"], y=hourly["conso_moy"], name="Référence", mode="lines+markers"))
    if "conso_optimisee_rl_moy" in hourly.columns:
        fig.add_trace(go.Scatter(x=hourly["heure"], y=hourly["conso_optimisee_rl_moy"], name="Après RL", mode="lines+markers"))
...
def page_optimisation_rl():
    ...
    header(
        PAGE_OPTIMISATION,
        "Actions et économies sur le parc filtré" if not admin else "Décisions NB3 et gains énergétiques",
    )
    ...
    with section("Actions par station"):
        render_actions_par_station(latest_per_station(df), show_savings=admin, per_mode=3)
