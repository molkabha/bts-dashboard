...
def _render_mapbox(scores: pd.DataFrame, template: str):
    ...
    if "action_actuelle" in plot_scores.columns:
        ...
        plot_scores["action_actuelle"] = (
            plot_scores["action_actuelle"].fillna("Maintien").astype(str)
        )
    ...
    if (
        is_admin()
        and "score_criticite" in plot_scores.columns
        and ("score_criticite" not in hover_cols)
    ):
    ...
    with st.expander("Comparaison gouvernorats / technologies", expanded=False):
        ...
        with c1:
            ...
            if "gouvernorat" in df.columns and "consommation_kwh" in df.columns:
                ...
                fig = px.bar(
                    by_gov, x="consommation_kwh", y="gouvernorat", orientation="h"
                )
        ...
        with c2:
            ...
            if "technologie" in df.columns:
                ...
                by_tech = (
                    df.groupby("technologie")
                    .agg(conso=("consommation_kwh", "mean"), qos=("score_qos", "mean"))
                    .reset_index()
                    .round(2)
                )
...
def page_vue_reseau():
    ...
    with section("Liste des stations"):
        ...
        sort_col = (
            "score_criticite"
            if is_admin() and "score_criticite" in scores.columns
            else "conso_moy"
        )
