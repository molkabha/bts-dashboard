...
from ui.page_helpers import (
    ...
    latest_per_station,
    load_dashboard_df,
    render_executive_report_export,
)
...
def page_accueil():
    ...
    if is_admin():
        ...
        with c2:
            ...
            kpi_card(
                "CO₂ évité", f"{float(kpis.get('co2_evite_t') or 0):.1f} t", "", "eco"
            )
        ...
        with c3:
            ...
            kpi_card(
                "Stations ECO",
                f"{float(kpis.get('pct_mode_eco') or 0):.1f}%",
                "Dernier mode / station",
                "eco",
            )
        ...
        with c3:
            ...
            kpi_card(
                "Mode ECO", f"{float(kpis.get('pct_mode_eco') or 0):.1f}%", "", "eco"
            )
    ...
    with c1:
        ...
        with section("Répartition des modes"):
            ...
            if "mode_operation" in df.columns and "station_id" in df.columns:
                ...
                fig.update_layout(
                    template=template, height=280, margin=dict(l=0, r=0, t=8, b=0)
                )
            ...
            if scores_df.empty:
                ...
                if "mode_actuel" not in top.columns and "mode_operation" in df.columns:
                    ...
                    modes = (
                        ...
                        .groupby("station_id")["mode_operation"]
                        .last()
                        if "timestamp" in df.columns
                        else df.groupby("station_id")["mode_operation"].first()
                    )

                    top = top.merge(
                        modes.rename("mode_actuel"),
                        left_on="station_id",
                        right_index=True,
                        how="left",
                    )
