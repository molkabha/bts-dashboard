...
class RapportPDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(
            0,
            10,
            "Tunisie Telecom - BTS Energy Management",
            align="C",
            new_x="LMARGIN",
            new_y="NEXT",
        )
...
def generate_report_pdf(kpis: dict, top_anomalies: list[dict] | None = None) -> bytes:
    ...
    conso = (
        f"{kpis.get('conso_totale_kwh', 0):,.0f} kWh"
        if pd.notna(kpis.get("conso_totale_kwh"))
        else "0 kWh"
    )

    eco_pct = (
        f"{kpis.get('economie_rl_pct', 0):.1f}%"
        if pd.notna(kpis.get("economie_rl_pct"))
        else "0.0%"
    )

    eco_dt = (
        f"{kpis.get('economie_dt', 0):,.0f} DT"
        if pd.notna(kpis.get("economie_dt"))
        else "0 DT"
    )

    co2 = (
        f"{kpis.get('co2_evite_t', 0):.1f} t"
        if pd.notna(kpis.get("co2_evite_t"))
        else "0.0 t"
    )

    qos = (
        f"{kpis.get('score_qos_moyen', 0):.3f}"
        if pd.notna(kpis.get("score_qos_moyen"))
        else "0.000"
    )
    ...
    anomalies = (
        f"{float(pct_anom):.1f}%"
        if pct_anom is not None and pd.notna(pct_anom)
        else "N/D"
    )
    ...
    *,
    ...
    if report.empty:
        ...
        station_sections = sorted(
            {
                s
                for s in work["Section"].astype(str).unique()
                if str(s).startswith("Station ")
            }
        )
        ...
        if station_sections:
            ...
            for section in station_sections[:40]:
                ...
                sub = {
                    str(r["Libelle"]): r["Valeur"]
                    for _, r in work[work["Section"] == section].iterrows()
                }
    ...
    if not alert_items:
        ...
        pdf.table_rows(
            ("Heure", "Station", "Gravite", "Message"), alert_rows, (28, 38, 28, 96)
        )
