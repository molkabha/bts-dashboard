...
def is_rate_limited(username: str) -> tuple[bool, Optional[int]]:
    ...
    return security_middleware.is_login_locked_out(
        username, MAX_LOGIN_ATTEMPTS, LOGIN_WINDOW_SECONDS
    )
...
def update_user_password(
    username: str, new_password: str, must_change_password: int = 0
) -> None:

    db_execute(
        "update_password", (password_hash(new_password), must_change_password, username)
    )
