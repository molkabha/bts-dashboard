...
def send_account_status_email(email: str, display_name: str, username: str, is_active: bool) -> tuple[bool, str]:
    host = smtp_config_value("host")
    port = int(smtp_config_value("port", "587"))
    smtp_user = smtp_config_value("user")
    smtp_password = smtp_config_value("password")
    ...
    if not host or not sender:
        return False, "Service email non configure."

    status_text = "active" if is_active else "desactive"
    msg = EmailMessage()
    msg["Subject"] = f"Statut de votre compte BTS EMS : {status_text.capitalize()}"
    msg["From"] = sender
    msg["To"] = email
    ...
    if is_active:
        ...
        body.append("Votre acces a l'application a ete suspendu. Veuillez contacter un administrateur pour plus d'informations.")
...
def send_account_deletion_email(email: str, display_name: str, username: str) -> tuple[bool, str]:
    ...
    msg = EmailMessage()
    msg["Subject"] = "Suppression de votre compte BTS EMS"
    msg["From"] = sender
    msg["To"] = email
