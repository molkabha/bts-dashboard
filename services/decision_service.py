...
class MoteurDecisionEnergie:
    ...
    def decider_mode(self, row: pd.Series) -> dict:
        ...
        score_qos = 1.0 if pd.isna(score_qos_raw) else float(score_qos_raw if score_qos_raw != "" else 1)
    ...
    def appliquer_sur_dataset(self, df_in: pd.DataFrame) -> pd.DataFrame:
        ...
        score_qos = pd.to_numeric(pd.Series(df.get("score_qos", 1), index=df.index), errors="coerce").fillna(1)
        ...
        df["risque_qos"] = np.select([score_qos >= 0.80, score_qos >= self.seuils["qos"]],
                                     ["Faible", "Modere"], default="Critique")
        return df
