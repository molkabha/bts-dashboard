...
class MoteurDecisionEnergie:
    ...
    def decider_mode(self, row: pd.Series) -> dict:
        ...
        score_qos = 0.0 if pd.isna(score_qos_raw) else float(score_qos_raw or 0)
        qos_ok = int(row.get("optimisation_qos_autorisee", score_qos >= self.seuils["qos"]))
        ...
        if score > self.seuils["critique_score"] or votes >= 5:
            ...
            and self.seuils["eco_heure_debut"] <= heure <= self.seuils["eco_heure_fin"]
            and qos_ok == 1
        ):
            mode = "ECO"
        else:
    ...
    def appliquer_sur_dataset(self, df_in: pd.DataFrame) -> pd.DataFrame:
        ...
        if df.empty:
            for col in ["mode_operation", "priorite", "action_principale", "eco_potentiel_pct", "risque_qos"]:
                df[col] = []
            return df
        # Extraction sécurisée des colonnes (gestion des colonnes manquantes comme Series)
        score = pd.to_numeric(pd.Series(df.get("anomalie_score_ensemble", 0), index=df.index), errors="coerce").fillna(0)
        ecart = pd.to_numeric(pd.Series(df.get("ecart_pct", 0), index=df.index), errors="coerce").abs().fillna(0)
        cpu = pd.to_numeric(pd.Series(df.get("charge_cpu_pct", 50), index=df.index), errors="coerce").fillna(50)
        heure = pd.to_numeric(pd.Series(df.get("heure", 12), index=df.index), errors="coerce").fillna(12).astype(int)
        votes = pd.to_numeric(pd.Series(df.get("nb_votes_anomalie", 0), index=df.index), errors="coerce").fillna(0).astype(int)
        score_qos = pd.to_numeric(pd.Series(df.get("score_qos", 1), index=df.index), errors="coerce").fillna(0)
        
        # Pour qos_ok, on récupère soit la colonne, soit le calcul par défaut
        qos_val = df.get("optimisation_qos_autorisee", score_qos >= self.seuils["qos"])
        qos_ok = pd.to_numeric(pd.Series(qos_val, index=df.index), errors="coerce").fillna(0).astype(int)
        critique = (score > self.seuils["critique_score"]) | (votes >= 5)
        attention = (ecart > self.seuils["critique_ecart"]) | ((score > self.seuils["eco_score"]) & (votes >= 2))
        eco = (
            (score < self.seuils["eco_score"])
            & (cpu < self.seuils["eco_cpu"])
            & heure.between(self.seuils["eco_heure_debut"], self.seuils["eco_heure_fin"])
            & (qos_ok == 1)
        )
        modes = np.select([critique, attention, eco], ["CRITIQUE", "ATTENTION", "ECO"], default="NORMAL")
        df["mode_operation"] = modes
        df["priorite"] = pd.Series(modes, index=df.index).map(lambda mode: self.MODES[mode]["priorite"])
        df["action_principale"] = pd.Series(modes, index=df.index).map(lambda mode: self.ACTIONS[mode][0])
        df["eco_potentiel_pct"] = np.where(df["mode_operation"].eq("ECO"), 20, 0)
        df["risque_qos"] = np.select([score_qos >= 0.80, score_qos >= self.seuils["qos"]], ["Faible", "Modere"], default="Critique")
        return df
