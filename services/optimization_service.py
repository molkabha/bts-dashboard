...
class StrategieOptimisation:
    ...
    def appliquer(self, df_in: pd.DataFrame) -> pd.DataFrame:
        ...
        # 1. Sleep mode nocturne (heures creuses 0h-5h)
        peut_sleep = (
            (qos_ok == 1)
            & (heure.between(0, 5))
        ...
        df.loc[peut_sleep, "economie_estimee_kwh"] = technologie.loc[peut_sleep].map(
            self.ECO_SLEEP_PAR_TECHNO).fillna(0.5)

        # 2. Reduction puissance emission (heures normales, charge faible-moyenne)
        peut_reduire = (
            (qos_ok == 1)
        ...
        # 3. Free cooling (temperature basse + vent)
        peut_free = (
        ...
        # 4. Optimisation adaptative IA (gain marginal toujours present sauf en mode critique)
        non_critique = self._col(df, "anomalie_score_ensemble", 0) < 0.60
        baseline_eco = conso * 0.03  # L'IA trouve toujours ~3% d'optimisation marginale
        already_optimized = df["economie_estimee_kwh"] > 0
        df.loc[non_critique & ~already_optimized,
               "economie_estimee_kwh"] = baseline_eco.loc[non_critique & ~already_optimized]
        df.loc[non_critique & ~already_optimized, "action_proposee"] = "optimisation_adaptative"

        # 5. Alertes (pas d'economie)
        alerte_2g = (
            technologie == "2G") & (
            self._col(
                df,
